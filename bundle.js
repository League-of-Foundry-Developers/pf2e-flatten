...
async function retrivePWL(){
	return await game.settings.get(settingsKey, "halflevel") ? game.packs.get('pf2e-flatten.pf2e-proficiency-without-level').getDocument(game.packs.get('pf2e-flatten.pf2e-proficiency-without-level').index.find(e => e.name === "1/2 PWL")._id) : game.packs.get('pf2e-flatten.pf2e-proficiency-without-level').getDocument(game.packs.get('pf2e-flatten.pf2e-proficiency-without-level').index.find(e => e.name === "PWL")._id);
}
...
const modifierName = game.settings.get(settingsKey, "halflevel") ? '1/2 Level Proficiency' :'Proficiency Without Level';
const modifierUnName = game.settings.get(settingsKey, "halflevel") ? 'Proficiency Without Level': '1/2 Level Proficiency' ;
